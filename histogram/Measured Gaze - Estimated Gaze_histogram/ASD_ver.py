...
# 保存ディレクトリ
save_dir = '../gaze_estimation_library/result(estimation_gaze)/ASD1/histogram/Measured Gaze - Estimated Gaze_histogram/vertical'
...
for start in range(0, end, duration):
    ...
    plt.ylabel('Second')
    plt.title(f'ASD 垂直方向の角度差 (Time {start}s-{end}s)')
    plt.grid(True)
    plt.xlim(-100, 100)  # 横軸の範囲を統一
    plt.ylim(0, 20)  # 縦軸の最大値を統一

    # ファイル名を生成して保存
    filename = os.path.join(save_dir, f'ASD_ver_histogram_{start}s-{end}s.png')
