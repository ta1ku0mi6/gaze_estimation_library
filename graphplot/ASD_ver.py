...
# 保存ディレクトリ
save_dir = '../gaze_estimation_library/result(estimation_gaze)/ASD1/vertical'
...
for start in range(start, end, interval):
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('(deg)')
    plt.title(f'ASD Vertical (Time {start}s-{end}s)')
    plt.legend()
    plt.grid(True)

    # ファイル名を生成して保存
    filename = os.path.join(save_dir, f'ASDplot_ver_{start}s-{end}s.png')
